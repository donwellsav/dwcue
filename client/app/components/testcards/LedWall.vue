  ...
  export default {
    ...
    computed: {
      infoStyle: function() {
        ...
        let top = (Math.min(this.info.rootHeight, this.config.led.rows*this.config.led.height)/2)
        if (this.info.rootHeight >= this.config.led.rows*this.config.led.height && this.config.led.height > 42) {
          top -= (offset * (1-this.config.led.rows%2))
        }
        
        return {
          left: Math.min(this.info.rootWidth, this.config.led.columns*this.config.led.width)/2 + 'px',
          top: top + 'px',
          fontSize: offset + 'px'
        }
      },
