...
> Section 1 tracks current source `7a0eee2`, whose package metadata is v2.6.14. This is not a published-installer claim; verify releases and the installed About version separately.
...
```
┌─────────────────────────────┐   TCP 4480 (token-authenticated REST + WS)   ┌──────────────────────────────┐
│  Client                     │ ◄───────────────────────────────────────► │  Server                      │
...
REST: `/api/health`, `/api/devices[...]`, `/api/cues[...]` (register/play/stop/gain/fade/ltc), `/api/transport/stop_all`, `/api/routing/{item_to_mixer,mixer_to_master,master_to_device}`, `/api/mixers`, `/api/fs/list`, `/api/upload`, `/api/project/{load,save}`, and `GET /api/project`.

`GET /api/health` is the sole public application route; every local and remote control route requires a token. REST uses a bearer header. Access-token query parameters are limited to browser `/ws` and `/api/media`. Browser origins match `LIVEPLAY_ALLOWED_ORIGINS` exactly (`http://localhost:3000` for source development); packaged Electron's opaque `Origin: null` remains token-authenticated.

WebSocket `/ws`: server→client `meters`, `cue_state` transitions, and `doc_patch` multi-client sync; client→server transport commands.
...
- **Runtime continuation and trigger acceptance**: `cue_to_continue` is a one-play, non-serialized override and never mutates saved `endBehavior`; Stop/remove/replay/Stop All/media replacement/project switch cancel it. **Set As Next** is server-authoritative (valid UUID or explicit clear), GO consumes it only after accepted playback, and failed/not-loaded targets remain armed. Play All preflights its selected subtree and starts nothing unless all selected descendants are ready; Stop All/Panic bypass readiness.
...
- **Sequencing and ducking**: audio `play-next` / `play-item` / `play-index` Start Behaviors execute with cycle fencing; Start Next is structural and does not consume global Up Next, while natural-end Audio End **Next** may honor it. A native `GroupRun` owns selected descendants/nested groups and fires its authored End Behavior exactly once after natural completion; the default is **Nothing**. Duck owners are generation-fenced, strongest-ceiling wins, and authored attack/release ramps restore recomputed base gain without audio-thread allocation.
- **Project lifetime and save ordering**: paged hydration is an explicit loading/ready/failed barrier. Failure leaves a read-only loaded surface with Retry and Stop All; Retry rejoins the current server header/pages rather than reopening the old file. Detached One Shot mutations carry `projectPath` / `projectEpoch` / `ownerSessionId` plain DTOs through Electron, are applied/persisted only by the primary renderer, and surface failures in the detached panel/popover. Saves wait for hydration, run through one latest-write queue and revision/identity fence, and retain dirty state on failure. Toggling Autosave either way force-saves the current document/preference; only subsequent off-mode edits are dirty-only. Its badge is off-mode-only and may hide on enable even if that forced save fails. Native Save As is serialized and atomically published; failure preserves the previous path/file.
...
- **Current creation/import UX**: **New Show** combines a name field with a read-only Location selected through **Choose…** and creates a native `.dwcue` document. Portable exports are ZIP-based `.dwcuepack` archives. Native `.dwcuepack` and legacy `.lpa` imports require exactly one root project document; the multiple-project selection branch is retained only for older-server response compatibility. **Import Project** converts legacy `.liveplay` / `.lpa` one-way while preserving original bytes. **Import Media** leads with local **Choose files**; advanced/server browsing is collapsed and unsupported non-media entries are disabled. The target is named **Play Next**. Unused One Shots stay collapsed unless explicitly shown.
- **Output and application lifecycle**: MIDI Master Volume remains the server's Global Master, distinct from output channel 0; its acknowledgement/readout mirrors the authoritative gain while all 32 output faders remain independent. Relaunch, exit and update preparation must remove client-owned AV diagnostics first and fail closed on removal error. **Close Client Only** removes that diagnostic but preserves the detached Program server and playback; stopping the server remains an explicit separate choice.
...
- **What remains, enlarged:** playlist (read-only rows, touch-friendly min ~56 px height),
  GO / panic (stop-all) transport bar, One Shots grid, master fader, **Play Next** display.
...
1. **New repo** `companion-module-donwells-cue` (scaffold with
   ...
   - **Variables:** current item name, elapsed/remaining time, Play Next name, master gain,
...
2. **DonWells Cue server additions** (small): a stable **external-control surface** —
   - `GET /api/state/summary` — compact machine-readable transport state (playing items with
     index, name, elapsed/duration, Play Next target) so Companion doesn't need the whole project JSON.
