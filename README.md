...
> **Current source:** package metadata is **v2.6.14** and this guide describes source commit **7a0eee2**. This is source provenance, not a published-installer claim: the release page is authoritative for release assets, and you must verify the installed About version before relying on a behaviour.
...
- **📊 See your levels** — real-time metering at every stage (per-cue, per-channel and master), shown in LUFS, dBFS, true-peak or RMS. The output console reports the server-authoritative **Global Master** gain separately from all 32 output-channel faders.
