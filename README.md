...
Apple Silicon Macs can also install from the project's Homebrew tap:

```sh
brew install donwellsav/tap/donwells-cue
```

This installs the same `arm64` DMG published on the release page, and `brew upgrade` picks up new versions (the tap's cask is regenerated by the release workflow). An Intel cask will follow once releases ship an `x64` DMG again.
